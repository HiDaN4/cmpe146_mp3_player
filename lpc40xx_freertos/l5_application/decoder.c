...
void vs_decoder__initialize(void) {
  ...
  int mode = vs__read_register(SCI_MODE);
}
...
pinMode(MP3_RESET, OUTPUT);
  */
  ...
  gpio_s mosi = gpio__construct_with_function(GPIO__PORT_1, 1, GPIO__FUNCTION_2);
  gpio__set_as_output(mosi);

  gpio_s scl = gpio__construct_with_function(GPIO__PORT_1, 0, GPIO__FUNCTION_2);
